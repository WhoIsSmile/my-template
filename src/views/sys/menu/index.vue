      ...
      <el-table :data="dbData" border style="width: 100%" row-key="id">
        <el-table-column type="expand">
          <template slot-scope="scope2">
            <el-table :data="scope2.row.childrenMenu" style="width: 100%" row-key="id" border stripe>
              ...
              <el-table-column label="操作" align="center">
                <template slot-scope="scope3">
                  ...
                  <el-button :loading="loading" size="mini" type="danger" @click="handleDelete(scope3.$index, scope3.row)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </template>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button :loading="loading" size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
    <Add :title="title" :edit-able="editAble" :parentData="parentData" :edit-data="editData" :showDialog="dialogFormVisible" v-on:hideAddDialog="hideDialog" />
  </div>
</template>
<script>
// icon 图标
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      this.$store.dispatch("sys/menu/list")
        .then(res => {
    ...
    initParentData() {
      ...
      for (var i = 0; i < this.dbData.length; i++) {
        var parentMenu = new ParentMenu(
          this.dbData[i].id,  this.dbData[i].menuName + "(二级菜单)"
        );
